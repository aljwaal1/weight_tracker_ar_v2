...
class Entry {
  ...
  static Entry decode(String raw) {
    final p = raw.split('|||');
    return Entry(value: p.isNotEmpty ? double.tryParse(p[0]) ?? 0 : 0, note: p.length > 1 ? p[1] : '', date: p.length > 2 ? DateTime.tryParse(p[2]) ?? DateTime.now() : DateTime.now());
  }
}
...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Future<void> load() async {
    ...
    final saved = p.getStringList('measure_log_v3');
    setState(() { entries = (saved == null || saved.isEmpty) ? [] : saved.map(Entry.decode).toList(); reminders = p.getBool('measure_reminders') ?? false; reminderTime = TimeOfDay(hour: p.getInt('measure_h') ?? 8, minute: p.getInt('measure_m') ?? 0); });
  }
  Future<void> save() async {
    final p = await SharedPreferences.getInstance();
    await p.setStringList('measure_log_v3', entries.map((e) => e.encode()).toList());
    await p.setBool('measure_reminders', reminders);
    await p.setInt('measure_h', reminderTime.hour);
    await p.setInt('measure_m', reminderTime.minute);
  }
  ...
  void addEntry() {
    final v = double.tryParse(valueCtrl.text.trim().replaceAll(',', '.'));
    if (v == null || v <= 0) return;
    setState(() { entries.insert(0, Entry(value: v, note: noteCtrl.text.trim(), date: DateTime.now())); valueCtrl.clear(); noteCtrl.clear(); });
    save();
  ...
  void deleteEntry(int i) {
    ...
    save();
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: const Text('تم حذف القياس'), action: SnackBarAction(label: 'تراجع', onPressed: () { setState(() => entries.insert(i, old)); save(); })));
  }

  void copyLog() {
    Clipboard.setData(ClipboardData(text: entries.map((e) => '${date(e.date)} - ${e.value.toStringAsFixed(1)} - ${e.note}').join('\n')));
